...
inline ArrD Matrix::kappa() const
{
  ...
  #pragma omp parallel
  {
    ...
    for ( size_t i = 0 ; i < m_type.size() ; ++i )
    {
      switch ( m_type[i] )
      {
        case Type::Elastic: a_K[i] = m_Elastic[m_index[i]].kappa(); break;
        case Type::Cusp   : a_K[i] = m_Cusp   [m_index[i]].kappa(); break;
        case Type::Smooth : a_K[i] = m_Smooth [m_index[i]].kappa(); break;
...
inline ArrD Matrix::mu() const
{
  ...
  #pragma omp parallel
  {
    ...
    for ( size_t i = 0 ; i < m_type.size() ; ++i )
    {
      switch ( m_type[i] )
      {
        case Type::Elastic: a_G[i] = m_Elastic[m_index[i]].mu(); break;
        case Type::Cusp   : a_G[i] = m_Cusp   [m_index[i]].mu(); break;
        case Type::Smooth : a_G[i] = m_Smooth [m_index[i]].mu(); break;
        default: std::runtime_error("Unknown material");
      }
    }
  }
...
inline void Matrix::setElastic(const ArrS &I, double kappa, double mu)
{
  // check input
  #ifndef NDEBUG
    // - shape
  ...
  m_Elastic.push_back(Elastic(kappa, mu));
}

// ----------------------------------- set Cusp material points ------------------------------------

inline void Matrix::setCusp(const ArrS &I,
  double kappa, double mu, const std::vector<double> &epsy, bool init_elastic)
{
  // check input
  #ifndef NDEBUG
    // - shape
  ...
  // store material definition
  m_Cusp.push_back(Cusp(kappa, mu, epsy, init_elastic));
}
  ...
  double kappa, double mu, const std::vector<double> &epsy, bool init_elastic)
{
  ...
  // store material definition
  m_Smooth.push_back(Smooth(kappa, mu, epsy, init_elastic));
}

// ---------------------------------- set Elastic material points ----------------------------------

inline void Matrix::setElastic(const ArrS &I, const ArrS &idx, const ColD &kappa, const ColD &mu)
{
  // check input
  #ifndef NDEBUG
    // - shape
    ...
    for ( size_t i = 0 ; i < m_type.size() ; ++i )
      if ( I[i] )
        assert( m_type[i] == Type::Unset );
    // - extent
    assert( idx.max() == kappa.size()-1 );
    // - consistency
    assert( kappa.size() == mu.size() );
  #endif
  ...
  for ( size_t i = 0 ; i < kappa.size() ; ++i )
    m_Elastic.push_back(Elastic(kappa(i), mu(i)));
...
inline void Matrix::setCusp(const ArrS &I, const ArrS &idx,
  const ColD &kappa, const ColD &mu, const MatD &epsy, bool init_elastic)
{
  // check input
  #ifndef NDEBUG
    // - shape
    ...
    // - consistency
    assert( kappa.size() == mu.size()      );
    assert( kappa.size() == epsy.shape(0) );
  #endif
  ...
  for ( size_t i = 0 ; i < kappa.size() ; ++i ) {
    ...
    m_Cusp.push_back(Cusp(kappa(i), mu(i), y, init_elastic));
  }
  ...
  const ColD &kappa, const ColD &mu, const MatD &epsy, bool init_elastic)
{
  ...
  for ( size_t i = 0 ; i < kappa.size() ; ++i ) {
    std::vector<double> y(epsy.item(i), epsy.item(i)+epsy.shape(1));
    m_Smooth.push_back(Smooth(kappa(i), mu(i), y, init_elastic));
  }
